...
var XCanvas3DGraphics = /** @class */ (function () {
    function XCanvas3DGraphics() {
        ...
        this.material = new THREE.MeshPhongMaterial({ color: 0xffffff, emissive: 0x111111, side: THREE.DoubleSide });
    }
...
(function (XCanvas3DGraphics) {
    ...
    var LineSegments = /** @class */ (function (_super) {
        ...
        function LineSegments(lines, material) {
            ...
            _this.lines = [];
            _this.lines = lines || [];
            _this.material = material || new THREE.LineBasicMaterial({ color: 0x888888, opacity: 1 });
            return _this;
        }
    ...
    var Rebar = /** @class */ (function (_super) {
        __extends(Rebar, _super);
        function Rebar(points, color) {
            var _this = _super.call(this) || this;
            _this.points = [];
            _this.points = points;
            _this.red = color[0];
        ...
        Rebar.prototype.Generate = function () {
            var vertices = [];
            var normals = [];
            var indices = [];
            this.GenerateRebars(this.points, this.radius, vertices, normals, indices);
            var geovertices = new Float32Array(vertices.length * 3);
            var j = 0;
            for (var i = 0; i < vertices.length; i++) {
                j = i * 3;
                geovertices[j + 0] = vertices[i][0];
                geovertices[j + 1] = vertices[i][1];
                geovertices[j + 2] = vertices[i][2];
            }
            var geonormals = new Float32Array(normals.length * 3);
            var geocolors = new Float32Array(normals.length * 3);
            for (var i = 0; i < normals.length; i++) {
                j = i * 3;
                geonormals[j + 0] = normals[i][0];
                geonormals[j + 1] = normals[i][1];
                geonormals[j + 2] = normals[i][2];
                geocolors[j + 0] = this.red;
                geocolors[j + 1] = this.green;
                geocolors[j + 2] = this.blue;
            }
            ...
            buffergeo.setAttribute('color', new THREE.Float32BufferAttribute(geocolors, 3));
            buffergeo.setIndex(indices);
            var mesh = new THREE.Mesh(buffergeo, this.material);
            return mesh;
        };
        Rebar.prototype.Assemble = function (vertex, normal, index, vertices, normals, indices) {
            var lastindex = vertices.length;
            for (var n = 0; n < vertex.length; n += 3)
                vertices.push([vertex[n], vertex[n + 1], vertex[n + 2]]);
            for (var n = 0; n < normal.length; n += 3)
                normals.push([normal[n], normal[n + 1], normal[n + 2]]);
            for (var n = 0; n < index.length; n++)
                indices.push(lastindex + index[n]);
        };
        Rebar.prototype.GenerateRebars = function (points, radius, vertices, normals, indices) {
            ...
            var point2;
            var point3;
            var line = new XLine3D();
            var geometry;
            var vertex, index, normal, lastindex;
            var length;
            var cpoints = [];
            var split = 4;
            var geopoints;
            for (var j = 0; j < points.length - 1; j++) {
                ...
                if (length !== 0) {
                    if (j !== 0) {
                        if (length > dia) {
                            point1 = line.GetStartOffset(dia4);
                            cpoints.push(new THREE.Vector3(point1.x, point1.y, point1.z));
                            if (cpoints.length > 4)
                                split = cpoints.length;
                            closedSpline.points = cpoints;
                            geometry = new THREE.TubeBufferGeometry(closedSpline, 1, radius, segments, false);
                            vertex = geometry.getAttribute("position").array;
                            ...
                            cpoints.push(new THREE.Vector3(point2.x, point2.y, point2.z));
                        }
                    }
                    if (j !== points.length - 1) {
                        if (points.length !== 2)
                            point3 = line.GetEndOffset(dia4);
                        else
                            point3 = point1;
                        if (length > dia) {
                            cpoints = [];
                            cpoints.push(new THREE.Vector3(point3.x, point3.y, point3.z));
                            cpoints.push(new THREE.Vector3(point2.x, point2.y, point2.z));
                        }
                        if (points.length !== 2)
                            point2 = point3;
                    ...
                    geopoints = [];
                    geopoints.push(new THREE.Vector3(point1.x, point1.y, point1.z));
                    geopoints.push(new THREE.Vector3(point2.x, point2.y, point2.z));
                    if (length > dia) {
                        closedSpline.points = geopoints;
                        geometry = new THREE.TubeBufferGeometry(closedSpline, 1, radius, segments, false);
                        vertex = geometry.getAttribute("position").array;
                        normal = geometry.getAttribute("normal").array;
                        index = geometry.getIndex().array;
    ...
    var Mesh = /** @class */ (function (_super) {
        ...
        Mesh.prototype.Add = function (geometry) {
            ...
            for (var n = 0; n < colors.length; n += 3)
                this.colors.push([colors[n], colors[n + 1], colors[n + 2]]);
        ...
        Mesh.prototype.Generate = function () {
            ...
            var geocolors = new Float32Array(this.colors.length * 3);
            for (var i = 0; i < this.colors.length; i++) {
                j = i * 3;
                geocolors[j + 0] = this.colors[i][0];
                geocolors[j + 1] = this.colors[i][1];
                geocolors[j + 2] = this.colors[i][2];
            }
