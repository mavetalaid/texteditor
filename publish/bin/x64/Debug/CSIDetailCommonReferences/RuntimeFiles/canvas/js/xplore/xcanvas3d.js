...
var XCanvas3D = /** @class */ (function (_super) {
    ...
    XCanvas3D.prototype.InitializeCamera = function () {
        this.camera = new THREE.PerspectiveCamera(50, 0.5 * this.aspect, 0.001, 10000);
    ...
    XCanvas3D.prototype.AddSunlight = function () {
        ...
        this.sunLight.shadow.camera.far = radius * 5;
        var obj = this.scene;
    ...
    XCanvas3D.prototype.ZoomAll = function (modelbounds) {
        ...
        var bz = (bounds.max.z + bounds.min.z) / 2;
        var center = new THREE.Vector3(bx, by, bz);
        var radius = Math.max(bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y, bounds.max.z - bounds.min.z) * 0.85;
        var lengthx = Math.abs(bounds.max.x - bounds.min.x);
        var lengthy = Math.abs(bounds.max.y - bounds.min.y);
        var lengthz = Math.abs(bounds.max.z - bounds.min.z);
        var ratiocanvas = this.canvas.width / this.canvas.height;
        var ratiomodel = lengthx / lengthy;
        if (lengthz > lengthx && lengthz > lengthy)
            radius = lengthz * 1.1;
        if (this.canvas.width < this.canvas.height)
            radius /= this.canvas.width / this.canvas.height;
        else if (ratiocanvas < ratiomodel)
            radius /= this.canvas.width / this.canvas.height;
        var fov = this.camera.fov * Math.PI / 180;
        var len = radius / (Math.sin(fov));
        this.camera.position.set(center.x - len, center.y - len, center.z + len / 2);
        this.camera.lookAt(center);
        var incline = Math.min(bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y, (bounds.max.x - bounds.min.x - bounds.max.y + bounds.min.y) / 2) * Math.cos(Math.PI / 4);
